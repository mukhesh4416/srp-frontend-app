...
function DragAttachment({refId}) {
  const [attachmentsList,setAttachmentsList] = useState([])
    ...
    const attachTabHeader = (att)=>{
        return <div>
            <div className='d-inline heading'>{att.documentName}</div>
            <FontAwesomeIcon title="Download" className='mx-1 float-end' icon={faDownload} onClick={(e)=>{downloadPdf(att.attachFile, att.documentName)}}/>
            <FontAwesomeIcon title="Delete" className='mx-1 float-end' icon={faTrash} onClick={(e)=>{deleteAttachment(att)}}/>
        </div>
    }
      ...
      <div className='p-2'>
            <Accordion activeIndex={0}>
                { attachmentsList.map((attachment) => 
                    <AccordionTab header={attachTabHeader(attachment)}>
                        <div className='p-1 pt-0'>
                            <iframe
                            title="PDF Viewer"
                            src={`data:application/pdf;base64,${attachment?.attachFile}`}
                            width="100%"
                            height="470px"
                            style={{ border: 'none' }}
                            />
